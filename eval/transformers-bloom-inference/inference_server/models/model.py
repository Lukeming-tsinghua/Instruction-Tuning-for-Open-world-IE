...
import transformers
from transformers import AutoConfig, AutoModelForCausalLM, AutoModelForSeq2SeqLM, AutoTokenizer, StoppingCriteriaList
...
from ..utils import (
    ...
    TokenizeRequest,
    TokenizeResponse,
    StoppingCriteriaCloseJson,
    PrefixConstrainedGeneration,
)
...
class Model:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.title_trie_path = args.title_trie_path
        self.type_trie_path = args.type_trie_path
        self.max_type_num = args.max_type_num
        self.min_type_num = args.min_type_num
        self.force_type = args.force_type

    def post_init(self, model_name: str, tokenizer_name) -> None:
        ...
        self.stopping_criteria = StoppingCriteriaList([StoppingCriteriaCloseJson(tokenizer=self.tokenizer)])
        self.prefix_allowed_tokens_fn = PrefixConstrainedGeneration(
            tokenizer=self.tokenizer,
            title_trie_path=self.title_trie_path,
            type_trie_path=self.type_trie_path,
            max_type_num=self.max_type_num,
            min_type_num=self.min_type_num,
    ...
    def generate(self, request: GenerateRequest) -> Union[GenerateResponse, Exception]:
        try:
            ...
            output = self.model.generate(
                ...
                prefix_allowed_tokens_fn=lambda batch_id, sents: self.prefix_allowed_tokens_fn(batch_id, sents),
            )
            ...
            if self.is_encoder_decoder:
                ...
                if request.remove_input_from_output:
                    ...
                    prefix_to_add = torch.tensor([[self.prefix_token_id]] * batch_size * request.num_return_sequences).to(self.input_device)
                    ...
                    generated_text = [i[1:].strip() for i in generated_text]
                else:
                    generated_text = self.tokenizer.batch_decode(output_tokens, skip_special_tokens=True)
