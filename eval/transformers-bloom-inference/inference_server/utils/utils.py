...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument(
        ...
        type=str,
        required=False,
        default="/harddisk/user/keminglu/bigscience_tokenizer",
        help="model name to use",
    )
    group.add_argument(
        "--model_class",
    ...
    group.add_argument("--max_type_num", type=int, default=None, help="max supported batch size")
    group.add_argument("--min_type_num", type=int, default=0, help="max supported batch size")
...
@run_rank_n
def write_file_rank_0(file_path, inputs, responses, anns) -> None:
    with open(file_path, "w") as f:
        for input, response, ann in zip(inputs, responses, anns):
            try:
