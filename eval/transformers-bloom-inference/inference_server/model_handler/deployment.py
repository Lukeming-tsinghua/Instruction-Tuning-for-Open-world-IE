...
class ModelDeployment:
    def __init__(self, args: argparse.Namespace, grpc_allowed: bool = False):
        ...
        if self.use_grpc_server:
            self.tokenizer = load_tokenizer(args.tokenizer_name)
    ...
    def _initialize_service(self, args: argparse.Namespace):
        ...
        if args.deployment_framework in [DS_INFERENCE, DS_ZERO]:
            ...
            cmd = f"inference_server.model_handler.launch " +\
            f"--model_name {args.model_name} --tokenizer_name {args.tokenizer_name} --title_trie_path {args.title_trie_path} --type_trie_path {args.type_trie_path} " + \
            f"--max_type_num {args.max_type_num} --min_type_num {args.min_type_num} " +\
            f"--deployment_framework {args.deployment_framework} --dtype {get_str_dtype(args.dtype)} --port {ports} --model_class {args.model_class}"
