...
qid2label = json.load(open(os.path.join("/harddisk/data/nlp_data/kb/wikidata/20230301/mapping", "sitelinks.enwiki.title.json")))
label2qid = {value: key for key, value in qid2label.items()}
prior_map = json.load(open("/harddisk/data/nlp_data/kb/wikipedia/20230301/enwiki-20230301/mention/entity_prior.json"))
...
src_file = "/harddisk/user/keminglu/pretrained_data_processed/wikipedia_with_mention_wo_title_simplified_eval/corpus"
output_dir = "/harddisk/user/keminglu/pretrained_data_processed/wikipedia_with_mention_wo_title_simplified_aug_eval"
raw_data = sc.textFile(src_file)

LOGGER.error("[Process Log] Processing:") 
raw_data.map(json.loads)\
        .flatMap(
            lambda x: augment_sample(
                x,
                prior_map.value,
                label2qid.value,
                qid2label.value,
